...
; Integer inputs
(assert (= age_individual 17))
(assert (= age_taxpayer 41))
(assert (= days_in_year 365))
(assert (= days_same_abode 214))
(assert (= days_with_other_claimant 151))

; Real inputs
(assert (= gross_income 4300.0))
(assert (= exemption_amount 5200.0))
(assert (= total_support 21000.0))
(assert (= support_from_taxpayer 13500.0))
(assert (= support_from_taxpayer_as_alimony 0.0))
(assert (= support_from_taxpayer_new_spouse_to_child 0.0))
(assert (= support_from_individual 2800.0))
(assert (= sheltered_workshop_income 0.0))
(assert (= agi_taxpayer 78000.0))
(assert (= agi_other_claimant 72000.0))
(assert (= highest_parent_agi 78000.0))
