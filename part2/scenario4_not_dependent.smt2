...
; Return filing
(assert (= filed_joint_return true))
...
; Integer inputs
(assert (= age_individual 22))
(assert (= age_taxpayer 52))
(assert (= days_in_year 365))
(assert (= days_same_abode 245))
(assert (= days_with_other_claimant 0))

; Real inputs
(assert (= gross_income 8700.0))
(assert (= exemption_amount 5200.0))
(assert (= total_support 24000.0))
(assert (= support_from_taxpayer 16500.0))
(assert (= support_from_taxpayer_as_alimony 0.0))
(assert (= support_from_taxpayer_new_spouse_to_child 0.0))
(assert (= support_from_individual 5500.0))
(assert (= sheltered_workshop_income 0.0))
(assert (= agi_taxpayer 110000.0))
